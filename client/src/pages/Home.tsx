      ...
      <Box sx={{ p: 3, maxWidth: 800, mx: "auto" }}>
        ...
        {tasks.filter(task => !task.isDeleted && !task.isCompleted && task.title.toLowerCase().includes(search.toLowerCase())).length === 0 ? (
          ...
          tasks.filter(task => !task.isDeleted && !task.isCompleted && task.title.toLowerCase().includes(search.toLowerCase())).map((task) => (
            <Card key={task.id} sx={{ mb: 2, transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.03)' } }}>
              <CardContent>
                <Typography variant="h6">{task.title}</Typography>
                <Typography variant="body2" color="text.secondary">
                  {expandedTaskId === task.id || task.description.split(' ').length <= 20
                    ? task.description
                    : `${task.description.split(' ').slice(0, 20).join(' ')}...`}
                  {task.description.split(' ').length > 20 && expandedTaskId !== task.id && (
                    <Button size="small" onClick={() => setExpandedTaskId(task.id)} sx={{ ml: 1, textTransform: 'none', padding: 0 }}>
                      Read more
                    </Button>
                  )}
                  {task.description.split(' ').length > 20 && expandedTaskId === task.id && (
                    <Button size="small" onClick={() => setExpandedTaskId(null)} sx={{ ml: 1, textTransform: 'none', padding: 0 }}>
                      Show less
                    </Button>
                  )}
                </Typography>
